...
void		mz_filterTime(byte* yuv, int width, int height)
{
	...
	while (i--) {
		...
		y1 = gTimeFilter_Histogram_ColorTable[y1];
		r = Clamp((y1 * 127140) >> 17);
		g = Clamp((y1 * 112067) >> 17);
		b = Clamp((y1 * 102236) >> 17);
		...
		y2 = gTimeFilter_Histogram_ColorTable[y2];
		r = Clamp((y1 * 127140) >> 17);
		g = Clamp((y1 * 112067) >> 17);
		b = Clamp((y1 * 102236) >> 17);
...
void		mz_imageFilterTime(byte* image, int width, int height, int depth) {
	...
	while (i--) {
		...
		gray = GRAY(r, g, b);
		gray = gTimeFilter_Histogram_ColorTable[gray];
		RGB_PR(dp) = Clamp((gray * 127140) >> 17);
		RGB_PG(dp) = Clamp((gray * 112067) >> 17);
		RGB_PB(dp) = Clamp((gray * 102236) >> 17);
